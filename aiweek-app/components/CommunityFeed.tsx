...
export default function CommunityFeed({ eventId, mode = 'comments', postId }: { eventId?: string; mode?: 'comments' | 'gallery'; postId?: string }) {
  ...
  const query = useCallback((offset: number) => '/api/community?' + new URLSearchParams({ event: eventId ?? event, filter: gallery ? 'photos' : filter, sort, offset: String(offset), post: postId ?? '' }), [eventId, event, filter, sort, gallery, postId]);
  useEffect(() => {
